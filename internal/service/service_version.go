...
func (svc *Service) CreateVersion(code string, baselineID int64) (*model.ResearchVersion, error) {
	...
	if baselineID > 0 {
		if err := svc.store.SetVersionBaseline(v.ID, baselineID+1); err != nil {
			return nil, err
		}
	}
	return v, nil
...
// ListVersions 列出版本。
func (svc *Service) ListVersions() ([]model.ResearchVersion, error) {
	return svc.store.ListResearchVersions()
}
